...
for i, fname in enumerate(stacked_fnames):
    ...
    kwargs = dict(eline_names=eline_list, plot_fit_model=plot_fit, fit_params=fit_params, \
                  stack_meth=stack_meth, norm_eline=norm_eline, offset=offset, bands=stacked_bands \
                 )


    
    fit_waves, fit = plot_spectra(rest_waves, luminosities, lum_errs, eline_rwave, save_model_txt=True, **kwargs)
    ...
    if stacked_bands == 'JH':
        # lt_4332 = np.where(rest_waves < 4332.)[0]
        # rest_waves, luminosities, lum_errs = np.delete(rest_waves, lt_4332), np.delete(luminosities, lt_4332), np.delete(lum_errs, lt_4332)
        ...
        fit_waves, fit   = plot_spectra(rest_waves[lte_4430], luminosities[lte_4430], lum_errs[lte_4430], eline_rwave, **kwargs)
        ...
        fit_waves, fit    = plot_spectra(rest_waves[gte_4800], luminosities[gte_4800], lum_errs[gte_4800], eline_rwave, **kwargs)
        filter_dict['JH']['Fit_Waves_Red'] = fit_waves
        filter_dict['JH']['Fit_Red'] = fit

        # if stack_meth == 'average':
        #     ax.set_ylim([-5.0e41, 5.0e41])
        ...
        fit_waves, fit        = plot_spectra(rest_waves[gte_6450], luminosities[gte_6450], lum_errs[gte_6450], eline_rwave, **kwargs)
        filter_dict['HK']['Fit_Waves'] = fit_waves
        filter_dict['HK']['Fit'] = fit

        plot_spectra(rest_waves[heI_wrange], luminosities[heI_wrange], lum_errs[heI_wrange], eline_rwave, **kwargs)
